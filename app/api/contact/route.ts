...
/*
 * NOTE: Resend is intentionally instantiated INSIDE the POST handler,
 * NOT at module level. If it were at module level, Next.js would run it
 * during "collecting page data" at build time — before Vercel injects
 * environment variables — causing:
...
function isRateLimited(ip: string): boolean {
  ...
  if (record.count >= RATE_MAX_HITS) return true
...
export async function POST(req: Request) {
  /* 1 ── Guard: env var must exist at request time ─────────── */
  if (!process.env.RESEND_API_KEY) {
    console.error('[contact] RESEND_API_KEY is not set')
    return NextResponse.json(
      { error: 'Server misconfiguration. Please contact me directly.' },
      { status: 500 }
    )
  }

  // Safe to instantiate here — env var is confirmed present
  ...
  const { error } = await resend.emails.send({
    /*
     * "from" must be a domain verified in your Resend dashboard.
     * onboarding@resend.dev works during testing but only delivers
     * to your own Resend account email. Once you verify a domain,
